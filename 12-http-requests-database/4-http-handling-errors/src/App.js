...
function App() {
  ...
  // !Default state is (null) because we don't have any error yet
  ...
  const fetchMoviesHandler = useCallback(async () => {
    try {
      ...
      // !Reset our (Error) state everytime we fetch the (database)
      setError(null)

      const response = await fetch('https://swapi.dev/api/films')

      // !Check first if our (response) is not ok
      if (!response.ok) {
      ...
      // !Getting the (error message) that we create by (throw new Error())
      setError(error.message)
    }

    // !Set our (Loading) state into (false) even we successful or catch an error
    setIsLoading(false)
  }, [movies])
