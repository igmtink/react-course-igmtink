...
function App() {
  ...
  const fetchMoviesHandler = useCallback(async () => {
    ...
    try {
      ...
      // !(Firebase) dummy (Database) api
      // !Creating / Getting (Database)
      const response = await fetch(
        'https://react-project-test-add-movie-default-rtdb.firebaseio.com/movies.json'
      )
      ...
      // !Converting (Nested Object) from our (Data) to (Array Object), so we can use (Array Map)
      const loadedMovies = []
      ...
      // !This we are not setting a new (Array Item) we are just fetching our (Database) from (Firebase)
  ...
  return (
    <main className="w-[640px] max-w-full mx-auto p-2 grid grid-cols-1 gap-4 h-screen">
      <section className="flex justify-center items-center self-end gap-4">
        <Button
      ...
      {addMovieShow && <AddMovie addMovie={addMovieHandler} />}
    </main>
  )
}
